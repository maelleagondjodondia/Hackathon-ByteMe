-- Replaced entire schema to match the exact provided SQL schema with UUID, correct column names
...
CREATE INDEX IF NOT EXISTS idx_joueur_email ON JOUEUR(email);
CREATE INDEX IF NOT EXISTS idx_joueur_elo ON JOUEUR(elo DESC);
CREATE INDEX IF NOT EXISTS idx_match1v1_date ON Match1V1(dateHeure DESC);
CREATE INDEX IF NOT EXISTS idx_match2v2_date ON Match2V2(dateHeure DESC);
